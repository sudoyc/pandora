...
路由、应用状态、图片服务和下载管理器只接收 `GalleryProvider`、`TagCatalog` 及 provider-neutral model/error。
...
Provider ID 统一规范化为小写安全路径组件；registry 拒绝未注册默认项，要求 factory 返回的
`provider_id` 与注册键一致，并在创建时验证完整 `GalleryProvider` 和 `TagCatalog` 协议，避免
adapter 切换后错误复用其他 provider 的持久化 workspace 或把缺失能力延迟为请求期故障。
...
adapter 内的 upstream model 和 provider-specific 标签目录实现不是 REST/CLI/WS v1，也不是 consumer 可导入的公共边界。
