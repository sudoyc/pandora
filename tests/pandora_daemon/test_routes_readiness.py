...
def test_readiness_without_credentials_is_deterministic_and_does_not_probe():
    app, api = _make_app(CredentialsConfig(igneous="sensitive-partial-cookie"))
    ...
    api.get_homepage.assert_not_awaited()
    api.search.assert_not_awaited()
    api.get_popular.assert_not_awaited()
    api.get_home_detail.assert_not_awaited()
...
def test_readiness_reports_ready_only_after_all_four_probes_succeed():
    ...
    _validate_schema(response.json())
    api.get_homepage.assert_awaited_once_with()
    api.search.assert_awaited_once()
    assert api.search.await_args.args[0].f_search
    api.get_popular.assert_awaited_once_with()
    api.get_home_detail.assert_awaited_once_with()
...
def test_readiness_classifies_failures_without_short_circuiting_or_leaking(
    ...
    app, api = _make_app(credentials)
    api.get_homepage.side_effect = exception
