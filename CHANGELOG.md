...
- Add a deterministic provider registry, provider-neutral domain and tag-catalog contracts, and a 26-endpoint replacement-provider architecture benchmark.
...
- Move the built-in ExHentai HTTP/parser and EhTagTranslation catalog implementations behind a nested adapter and isolate non-default provider databases, download state, and libraries while preserving default paths and public contract v1.
...
- Reject unsafe provider IDs, unregistered defaults, factory identity mismatches, and incomplete provider or tag-catalog implementations before selecting a persistent provider workspace.
